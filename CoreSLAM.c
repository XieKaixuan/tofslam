...
void
ts_state_init(ts_state_t *state, ts_map_t *map, ts_laser_parameters_t *laser_params, ts_position_t *position, double sigma_xy, double sigma_theta, int hole_width, int direction)
{
    ts_random_init(&state->randomizer, 0xdead);
    state->map = map;
    //state->params = *params; //Don't needed because we don't have wheels
